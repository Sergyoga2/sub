  ...
  return (
    <section className={cn("h-full w-full overflow-hidden py-32", className)}>
      <div className="container mx-auto border-t border-b border-dashed">
        <div className="relative mx-auto flex w-full max-w-5xl flex-col items-center justify-center border border-t-0 border-dashed px-5 py-12 text-center">
          <p className="flex items-center gap-3 text-sm text-muted-foreground">
            <span className="inline-block size-2 rounded bg-green-500" />
            {badgeText}
          </p>
          <div className="relative mt-3 mb-7 w-full max-w-xl text-center text-5xl font-medium font-semibold tracking-tighter md:mb-10 md:text-6xl lg:mb-0 lg:text-7xl">
            <h1 className="relative z-10 inline">
              {headingStart} <br className="block md:hidden" />
              {headingSecondLine && headingSecondLine}
            </h1>
            <ContainerTextFlip
              className="absolute left-1/2 -translate-x-1/2 text-4xl font-medium font-semibold tracking-tighter md:bottom-4 md:text-5xl lg:-bottom-4 lg:text-7xl"
